# xhs_downloads
提供便捷的收集素材应用工具，使用更加安全和人性化。优先下载高清原图和视频，免费按大家提出的意见进行优化！随时欢迎大家加入QQ群进行交流：1064541466

提示：当遇到下载问题时请及时反馈，本人会以最快的时间解决并更新新版本！！

使用方式（当前仅发布了Win版本）： https://github.com/golordmanji-stack/xhs_downloads/releases

如有需要Mac版本的可以加群或者给我留言~

1. 模拟真实浏览器操作，实现登录功能，并且支持自动登录功能。
2. 支持对关键词搜索记录和账号收藏记录自动保存浏览记录功能。
3. 支持通过数据筛选，勾选想要的素材并导出数据功能，包括文本、图片以及视频(原高清视频)等素材。

工具截图：

<img width="800" height="528" alt="Pasted Graphic" src="https://github.com/user-attachments/assets/4af0fb4f-8b11-42fc-95a1-7868d31b2b01" />
<img width="800" height="551" alt="Pasted Graphic 1" src="https://github.com/user-attachments/assets/b608edd5-7fb7-48c9-b255-a8a2eb886a1a" />
<img width="800" height="528" alt="Pasted Graphic 2" src="https://github.com/user-attachments/assets/bc3700ca-3a08-4d83-8c60-ab43a0d072c9" />
<img width="800" height="528" alt="WX20251127-091814@2x" src="https://github.com/user-attachments/assets/c2da355a-5638-49a3-b00d-4a439ea1b00c" />

操作步骤：

1. 下载BrowserTab_V1.06.zip文件
2. 解压BrowserTab_V1.06.zip，打开文件BrowserTab_V1.06，双击BrowserTab.exe启动
3. 登录xhs后，通过关键词搜索或者浏览个人收藏目录生成筛选数据
4. 点击数据筛选按钮，通过搜索过滤或者直接勾选数据，选择导出选中即可完成素材下载功能。

声明：本工具无任何商业用途，只是为了提供一个便于大家收集素材的小工具。如果大家还有什么需求，欢迎加QQ群：1064541466



